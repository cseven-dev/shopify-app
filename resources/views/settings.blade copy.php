...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-6">
        ...
        <x-nav-link
            ...
            class="ml-4 inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-green-600 hover:bg-white hover:text-green-700 transition">
            {{ __('View Full Logs') }}
        </x-nav-link>
    </div>
    {{-- Messages --}}
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
        ...
        <form method="POST" action="{{ route('settings.deleteClient') }}">
            @csrf
            <button type="submit" class="w-full bg-red-400 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                Delete Client
            </button>
        </form>
