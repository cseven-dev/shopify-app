                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <ul class="space-y-1 max-h-96 overflow-y-auto">
                            @forelse($logFiles as $logFile)
                                <li>
                                    <a href="{{ route('import.logs', ['log' => $logFile, 'shop' => $shopSlug ?? session('shop')]) }}"
                            ...
                            @empty
                                <li class="text-gray-500 px-3 py-2">No log files found for this shop</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex justify-between items-center mb-3">
                                <h3 class="text-lg font-semibold">
                                    Import Log: 
                                    {{ \Carbon\Carbon::createFromFormat('Y-m-d_H-i-s', str_replace(['import_log_', '.log'], '', basename($selectedLog)))->format('M d, Y H:i') }}
                                </h3>
